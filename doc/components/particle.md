...
The most basic usage of `Particle` with `BasicGame` would look as following:

```dart
import 'particle_prefab.dart';
...
More examples of using these behaviors together are available [here](/doc/examples/particles/lib/main.dart). All the implementations are available in [particles](/lib/src/particles) folder in Flame sources.
