...
class Bgm extends WidgetsBindingObserver {
  ...
  void play(String filename, {double volume}) async {
    ...
    isPlaying = true;
    audioPlayer = await Assets.audioCache.loopLongAudio(
      filename,
      volume: volume,
    );
  }
  ...
  Future<File> load(String file) => Assets.audioCache.load(file);

  /// Prefetch a list of audios and store them in the cache.
  ///
  /// Alias of `AudioCache.loadAll();`.
  Future<List<File>> loadAll(List<String> files) => Assets.audioCache.loadAll(files);

  /// Clears the file in the cache.
  ///
  /// Alias of `AudioCache.remove();`.
  void remove(String file) => Assets.audioCache.remove(file);

  /// Clears all the audios in the cache.
  /// Alias of `AudioCache.clear();`.
  ///
  void clear() => Assets.audioCache.clear();
